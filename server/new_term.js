...
const main = async () => {
  ...
  switch(response.choice) {
    ...
    case 'Y':
      ...
      const query = sqlstring.format('SELECT email, name FROM lockers WHERE status = ?', 'closed');
        ...
        if(error) {
          ...
          results.forEach(row => {
            sendmail({
              ...
              '<p>You are recieving this email because you have a locker reservation with us.</p>'+
          ...
          const query2 = sqlstring.format('UPDATE lockers SET status = ? WHERE status = ?', ['pending', 'closed']);
